...
from alameda_transformers import TRANSFORMERS
...
def read_raw(path, nfiles):
    path = Path("data") / "raw" / path / "RCV Cast Vote Record (CVR) JSON Files"
    print(path)
    path = list(path.glob("CVR_Export_*.zip"))[0]
    with zipfile.ZipFile(path, "r") as zip_file:
        ...
        for zip_info in zip_file.filelist:
            if nfiles is not None and idx == nfiles:
                break

            if not zip_info.filename.startswith("CvrExport"):
                continue
